...
const useHelper = () => {
  ...
  const { students, teachers, subjects } = useSelector(
  ...
  function handleInputOnChangeList(event: any) {
    ...
    if (name === "students") {
      if (
        !list.students
          .map((m: any) => `${m.name.first} ${m.name.last}` === value)
          .includes(true)
      ) {
        list.students.push(
    ...
    if (name === "subjects") {
      if (!list.subjects.map((m: any) => m.name === value).includes(true)) {
        list.subjects.push(subjects.find((m: any) => m.name === value));
      } else {
  ...
  function deleteFromList(event: any) {
    if (
      list.students.map((m: any) => m._id === event.target.value).includes(true)
    ) {
      let copy = list.students.filter((p: any) => p._id !== event.target.value);
      setList({ ...list, students: copy });
    }
    if (
      list.teachers.map((m: any) => m._id === event.target.value).includes(true)
    ) {
      let copy = list.teachers.filter((g: any) => g._id !== event.target.value);
